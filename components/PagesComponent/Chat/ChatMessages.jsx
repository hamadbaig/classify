...
const renderMessageContent = (message, isCurrentUser) => {
  const baseTextClass = isCurrentUser
    ? "text-white bg-primary p-2 rounded-md"
    : "text-black bg-border p-2 rounded-md";
  ...
  switch (message.message_type) {
    case "audio":
      return (
        <audio
          src={message.audio}
          controls
          className={`block rounded-md border-2 ${audioStyles}`}
    ...
    default:
      return <p className={baseTextClass}>{message?.message}</p>;
  }
};
                ...
                return (
                  <Fragment key={message?.id}>
                    ...
                    {message.sender_id === userId ? (
                      <div
                        className="flex flex-col gap-1 w-fit max-w-[80%] self-end"
                        key={message?.id}
                      >
                      ...
                      <div
                        className="flex flex-col gap-1 w-fit max-w-[80%]"
                        key={message?.id}
                      >
