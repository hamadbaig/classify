...
import { extractYear, placeholderImage, t } from "@/utils";
...
const SellerDetailCard = ({ productDetails, setProductDetails }) => {
  ...
  const placeholder_image = systemSettingsData?.placeholder_image;
  ...
  const isAllowedToMakeOffer =
    productDetails?.price > 0 && !productDetails?.is_already_offered;
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      router.push("/chat?activeTab=buying&chatid=" + data?.id);
    } catch (error) {
      ...
      <div className="flex items-center rounded-lg border flex-col">
        ...
        <div className="flex gap-2 justify-between w-full items-center p-4">
          <div className="flex gap-2.5 items-center max-w-[90%]">
            <Image
              onClick={() => router.push(`/seller/${productDetails?.user?.id}`)}
              src={
                productDetails?.user?.profile
                  ? productDetails?.user?.profile
                  : placeholder_image
              }
              alt="Seller Image"
